...
- **401 from Solr (`require authentication`, Solr URL in logs has no `user:pass@`):** **`DATAVERSE_SOLR_HOST`** (and **`SOLR_LOCATION`**) are **not** used if the **database** already has **`:SolrHostColonPort`** — that API setting **wins** over MicroProfile / env (see `SystemConfig.getSolrHostColonPort()` in IQSS Dataverse). Installers and **configbaker** often persist **`solr.solr.svc.cluster.local:8983`** with **no** basic-auth prefix, so Payara keeps calling Solr anonymously while your Solr chart enforces auth. **Fix:** as a superuser, **`PUT /api/admin/settings/:SolrHostColonPort`** with body **`SolrUser:SolrPassword@solr.solr.svc.cluster.local:8983`** (same values as **`SOLR_ADMIN_*`**; [URL-encode](https://en.wikipedia.org/wiki/Percent-encoding) **`:`**, **`@`**, **`#`**, **`%`**, etc. in the password if needed), then **restart Dataverse** (roll the Deployment). Alternatively clear that DB setting so env-only config applies (only if you know nothing else depends on it). **`solrInit`** / curl still need correct **`SOLR_ADMIN_*`** for ZK upload and collection APIs.  
- **401 with no `:SolrHostColonPort` row in `setting`:** the **gdcc** **`init_2_configure.sh`** only turns **lowercase** **`dataverse_*`** env vars into Payara **`create-system-properties`**; it does **not** read **`DATAVERSE_SOLR_HOST`** (uppercase). MicroProfile can still miss or order behind profile defaults, so searches hit Solr with no credentials. **`ops/besties-deploy.tmpl.yaml`** therefore sets **`dataverse_solr_host`**, **`dataverse_solr_port`**, and **`dataverse_solr_core`** (same values as the **`DATAVERSE_SOLR_*`** entries) so Payara gets **`dataverse.solr.*`** at boot. After changing secrets or this block, **roll the Deployment** (Payara reads these at startup).  
