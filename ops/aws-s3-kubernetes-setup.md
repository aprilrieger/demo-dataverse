...
This stack configures Dataverse’s S3 file store via **`006-s3-aws-storage.sh`**, which the Helm chart embeds when **`awsS3.enabled`** is true and mounts at **`/opt/payara/scripts/init.d/006-s3-aws-storage.sh`**. That path is where the **`gdcc/dataverse` base image entrypoint** sources scripts **before** Payara starts (see [Base image — Entry & Extension Points](https://guides.dataverse.org/en/latest/container/base-image.html#base-entrypoint)). The script runs only when **`aws_bucket_name`** is set (the chart sets it from **`awsS3.bucketName`**) and expects AWS CLI-style files under **`/secrets/aws-cli/.aws/`** (`credentials` and `config`). The chart mounts your Kubernetes **Secret** there when **`awsS3.enabled`** is true and sets **`AWS_SHARED_CREDENTIALS_FILE`** / **`AWS_CONFIG_FILE`** to those paths so the **Payara process (runs as user `payara`, not root)** can use the AWS SDK without copying credentials under **`/root/.aws`**.

The script does **not** call **`asadmin create-jvm-options`**: the DAS is not listening on **`localhost:4848`** during **`init.d`**. It rewrites **`POSTBOOT_COMMANDS_FILE`** with **`create-system-properties`** lines (same mechanism as **`init_2_configure.sh`**) so Payara applies S3 settings during domain boot.
